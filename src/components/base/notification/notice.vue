...
<script>
    import RenderCell from '../render'; // render function组件
    export default {
        components: {
            RenderCell
        },
        ...
        methods: {
            ...
            close () {
                // 关闭提醒的时候清除定时器，调用props上的onclose方法，并调用父组件的close方法
            ...
            handleLeave (el) {
                if (this.type === 'message') {
                    ...
                    if (document.getElementsByClassName('vu-message-notice').length !== 1) {
                        el.style.height = 0;
